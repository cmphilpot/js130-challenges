class BeerSong {
  ...
  static verse (number) {
    return BeerSong.lyrics(number, number);
  }

  static verses (start, finish) {
    return BeerSong.lyrics(start, finish);
  }

  static lyrics (start = 99, stop = 0) {
    let wholeSong = ``;

    for (let descendingNumber = start; descendingNumber >= stop; descendingNumber -= 1) {
      let singularOrPlural = descendingNumber === 1 ? 'bottle' : 'bottles';
      let oneOrIt = descendingNumber === 1 ? 'it' : 'one';

      if (descendingNumber === 0) {
        wholeSong += `No more bottles of beer on the wall, no more bottles of beer.\n`
        wholeSong += `Go to the store and buy some more, 99 bottles of beer on the wall.\n`;
        break;
      }
      wholeSong += `${descendingNumber} ${singularOrPlural} of beer on the wall, ${descendingNumber} ${singularOrPlural} of beer.\n`;
